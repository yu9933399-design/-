...
@WebServlet("/merchant/message/*")
public class MerchantMessageServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            if ("/list".equals(path) || path == null) {
                ...
                req.getRequestDispatcher("/jsp/merchant/message/list.jsp").forward(req, resp);
                ...
                if (otherUser == null) {
                    resp.sendRedirect(req.getContextPath() + "/merchant/message/list");
                    return;
                }
                req.setAttribute("otherUser", otherUser);
                List<Message> messages = messageService.getConversation(user.getUserId(), otherId);
                req.setAttribute("messages", messages);
                req.getRequestDispatcher("/jsp/merchant/message/chat.jsp").forward(req, resp);
            }
        } catch (Exception e) {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            if ("/send".equals(path)) {
                ...
                resp.sendRedirect(req.getContextPath() + "/merchant/message/chat?userId=" + receiverId);
            }
        } catch (Exception e) {
            req.setAttribute("error", e.getMessage());
            resp.sendRedirect(req.getContextPath() + "/merchant/message/list");
