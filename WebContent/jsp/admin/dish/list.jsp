...
<%-- 分页导航：总页数>1时显示，链接携带page/keyword/categoryId/status/merchantId参数，刷新后保留当前页码和全部筛选条件 --%>
<c:if test="${totalPages > 1}">
<nav class="mt-3"><ul class="pagination justify-content-center">
<li class="page-item ${currentPage <= 1 ? 'disabled' : ''}"><a class="page-link" href="${pageContext.request.contextPath}/admin/dish?page=${currentPage - 1}<c:if test='${not empty keyword}'>&keyword=${keyword}</c:if><c:if test='${categoryId != null}'>&categoryId=${categoryId}</c:if><c:if test='${status != null}'>&status=${status}</c:if><c:if test='${param.merchantId != null}'>&merchantId=${param.merchantId}</c:if>">上一页</a></li>
<c:forEach begin="1" end="${totalPages}" var="i">
<li class="page-item ${currentPage == i ? 'active' : ''}"><a class="page-link" href="${pageContext.request.contextPath}/admin/dish?page=${i}<c:if test='${not empty keyword}'>&keyword=${keyword}</c:if><c:if test='${categoryId != null}'>&categoryId=${categoryId}</c:if><c:if test='${status != null}'>&status=${status}</c:if><c:if test='${param.merchantId != null}'>&merchantId=${param.merchantId}</c:if>">${i}</a></li>
</c:forEach>
<li class="page-item ${currentPage >= totalPages ? 'disabled' : ''}"><a class="page-link" href="${pageContext.request.contextPath}/admin/dish?page=${currentPage + 1}<c:if test='${not empty keyword}'>&keyword=${keyword}</c:if><c:if test='${categoryId != null}'>&categoryId=${categoryId}</c:if><c:if test='${status != null}'>&status=${status}</c:if><c:if test='${param.merchantId != null}'>&merchantId=${param.merchantId}</c:if>">下一页</a></li>
</ul></nav></c:if>
</div></div>
