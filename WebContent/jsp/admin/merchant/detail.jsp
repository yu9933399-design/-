...
<style>
.dish-tooltip{position:relative;cursor:pointer}
.dish-tooltip .tip-box{display:none;position:absolute;left:50%;transform:translateX(-50%);bottom:100%;background:#fff;border:1px solid #ddd;border-radius:8px;padding:12px;width:220px;box-shadow:0 4px 12px rgba(0,0,0,.15);z-index:100;margin-bottom:8px}
.dish-tooltip:hover .tip-box{display:block}
.tip-box img{width:100%;height:100px;object-fit:cover;border-radius:6px;margin-bottom:6px}
.tip-box .tip-name{font-weight:600;font-size:14px;margin-bottom:2px}
.tip-box .tip-desc{font-size:12px;color:#999}
.tip-box .tip-price{font-size:16px;font-weight:700;color:#FF7800;margin-top:4px}
</style>
</head>
...
<!-- Tab 切换：通过URL参数?tab=dishes|orders保持刷新后的Tab状态 -->
<ul class="nav nav-tabs mb-3" id="merchantTab">
<li class="nav-item"><a class="nav-link ${empty param.tab || param.tab == 'dishes' ? 'active' : ''}" href="${pageContext.request.contextPath}/admin/merchant/detail?id=${merchant.userId}&tab=dishes"><i class="fas fa-utensils me-1"></i>菜品管理（${dishes.size()}）</a></li>
<li class="nav-item"><a class="nav-link ${param.tab == 'orders' ? 'active' : ''}" href="${pageContext.request.contextPath}/admin/merchant/detail?id=${merchant.userId}&tab=orders"><i class="fas fa-receipt me-1"></i>订单管理（${orders.size()}）</a></li>
</ul>

<!-- 菜品 Tab -->
<div id="tabDishes" style="${param.tab == 'orders' ? 'display:none;' : ''}">
...
<tbody>
<c:forEach items="${dishes}" var="d"><tr>
<td><div class="dish-tooltip"><c:out value="${d.dishName}"/>
<div class="tip-box">
<c:if test="${not empty d.imageUrl}"><img src="${pageContext.request.contextPath}/${d.imageUrl}"></c:if>
<div class="tip-name"><c:out value="${d.dishName}"/></div>
<div class="tip-desc"><c:out value="${d.description}" default="暂无描述"/></div>
<div class="tip-price">&#165;<fmt:formatNumber value="${d.price}" pattern="#,##0.00"/></div>
</div></div></td>
<td class="text-primary fw-bold">&#165;<fmt:formatNumber value="${d.price}" pattern="#,##0.00"/></td>
<td><c:out value="${d.categoryName}"/></td>
<td>${d.stock}</td>
...
<!-- 订单 Tab -->
<div id="tabOrders" style="${param.tab != 'orders' ? 'display:none;' : ''}">
<div class="card border-0 shadow-sm"><div class="card-body">
